...
export class FriendsPaginator {
  ...
  renderCallList(): string {
    const totalPages = this.getTotalPages();

    if (!this.friends.length) {
      return M.EMPTY_CALL_LIST;
    }

    let text = M.CALL_LIST_HEADER;
  ...
  getCallListKeyboard(): InlineKeyboard {
    const totalPages = this.getTotalPages();
    const friendsOnPage = this.getPageFriends();
    const keyboard = new InlineKeyboard();
    ...
    friendsOnPage.forEach((friend) => {
      keyboard
        .text(
          ...
          `call_friend_${friend.telegramId}`
        )
        .row();
    });
    ...
    if (totalPages > 1) {
      if (this.page > 1)
        keyboard.text("⬅️", `call_friends_page_${this.page - 1}`);
      if (this.page < totalPages)
        keyboard.text("➡️", `call_friends_page_${this.page + 1}`);
      keyboard.row();
    }
    keyboard.text("Назад ⬅️", "back-to-profile");
